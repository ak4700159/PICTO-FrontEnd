...
class PhotoStoreApi {
  ...
  Future<bool> deletePhoto(int photoId) async {
    try {
      final response = dio.delete('$baseUrl/photos/$photoId', queryParameters: {"userId": UserManagerApi().ownerId});
      return true;
    } catch (e) {
  ...
  Future<List<Photo>> getFrames() async {
    try {
      final response = await dio.get('$baseUrl/photos/frames', queryParameters: {"userId": UserManagerApi().ownerId});
  ...
  Future<bool> uploadPhotoInFrame({required int photoId, required Uint8List image}) async {
    try {
      final formData = FormData.fromMap({
        'file': MultipartFile.fromBytes(image, contentType: MediaType('image', 'jpeg'), filename: "$photoId.jpg"),
        'request': MultipartFile.fromString(jsonEncode({"frameActive": false, "sharedActive": false}),
