...
class _MarkerListBottomSheetState extends State<MarkerListBottomSheet> {
  ...
  Future<void> _startDownloadAllImages() async {
    ...
    for (String locationName in locationNames) {
      List<Photo> temp = await PhotoManagerApi()
          .getRandomPhotosByLocation(locationName: locationName, count: 10, eventType: "random");
      aroundPhotos.addAll(
          temp.map((p) => PictoMarker(photo: p, type: PictoMarkerType.aroundPhoto)).toList());
    }

    // 중복 요소 제거
    aroundPhotos.removeWhere(
        (aroundPhoto) => widget.markers.any((p) => p.photo.photoId == aroundPhoto.photo.photoId));
                      ...
                      return Align(
                        ...
                        child: GestureDetector(
                          onTap: () async {
                            ...
                            User? user = await UserManagerApi()
                                .getUserByUserId(userId: (marker.photo.userId!));
                          ...
                          child: SizedBox(
                            ...
                            child: Column(
                              children: [
                                ...
                                Text(
                                  formatDateKorean(marker.photo.updateDatetime ?? 0)
                                      .substring(0, "0000년 00월 00일".length),
                                  style: TextStyle(
                                    fontFamily: "NotoSansKR",
                                    fontSize: 10,
